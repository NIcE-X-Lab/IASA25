...
from run_experiment.run_experiment_cls import run_experiment
...
def run_experiment_with_config(config: dict):
    ...
    for feature_type in input_features:
        ...
        for batch in train_loader:
            ...
            if x is not None:
                B, T, D = tuple(x.shape)
                print(f"[DEBUG] CLS input feature '{feature_type}' first batch shape: (B={B}, T={T}, D={D})")
                input_dim = D
            else:
                ...
                if available_features:
                    ...
                    B, T, D = tuple(x.shape)
                    input_dim = D
                    print(f"[DEBUG] CLS fallback feature: {available_features[0]}, shape: (B={B}, T={T}, D={D})")
                else:
                    raise ValueError("No valid features found in batch!")
            break
        
        print(f"\n====== Running Experiment ======")
        metrics = run_experiment(train_loader, val_loader, test_loader, input_dim=input_dim, epochs=n_epoch, loss_plt_dir=loss_plt_dir, per_file_dir=per_file_dir, metrics_dir=metrics_dir, cm_dir=cm_dir, fold_idx=0, model_type=model, feature_type=feature_type, device=device, **model_param_dict)
        all_metrics.append(metrics)
        
    # Final summary (simple aggregate over folds)
    accs = [m.get("accuracy") for m in all_metrics if m.get("accuracy") is not None]
    aucs = [m.get("auc") for m in all_metrics if m.get("auc") is not None]
    mean_acc = sum(accs) / len(accs) if accs else float('nan')
    mean_auc = sum(aucs) / len(aucs) if aucs else float('nan')
    print("\n=== Aggregated Metrics (simple) ===")
