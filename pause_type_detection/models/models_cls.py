...
from models_reg import LSTM as LSTMBackbone

def get_model(model_type, model_params, input_dim, output_dim=4):
    if model_type == "vgg16":
...
class VGG16(nn.Module):
    def __init__(self, input_channels=1, num_classes=4, dropout=0.3):
...
class GRU(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, attention_dim=None, output_dim=4, bidirectional=True, num_layers=1, use_attention=False, target_length=750):
        super().__init__()
    ...
    def forward(self, x, return_repr=True):
        ...
        if return_repr:
            ...
            out_logits = self.classifier(feat)
            return out_logits
...
class TwoStageCNNLSTM(nn.Module):
    def __init__(self, hidden_dim_stage2: int = 128, target_length: int = 750, num_layers_stage2: int = 2, bidirectional_stage2: bool = True, kernel_size_conv: int = 3, padding_conv: int = 1, stride_conv: int = 1, dilation_conv: int = 1, bias_conv: bool = True, groups_conv: int = 1, fused_feature: str = "mfb", num_classes: int = 4, dropout_stage2: float = 0.3):
        super().__init__()
        self.break_predictor = LSTMBreakDetector(input_dim=40, hidden_dim=128, attention_dim=64, num_layers=2, dropout=0.3, bidirectional=True, target_length=750)
    ...
    def forward(self, mfb_features: torch.Tensor, wav2vec2_features: torch.Tensor, mfcc_features: torch.Tensor):
        ...
        logits, _, _, _ = self.cnnlstm(fused, lengths)
        return break_probs, logits


class TwoStageGRU(nn.Module):
    def __init__(self, hidden_dim_stage2: int = 128, target_length: int = 750, num_layers_stage2: int = 1, bidirectional_stage2: bool = True, use_attention_stage2: bool = False, attention_dim_stage2: int | None = None, fused_feature: str = "mfb", num_classes: int = 4, dropout_stage2: float = 0.3):
        super().__init__()
        self.break_predictor = LSTMBreakDetector(input_dim=40, hidden_dim=128, attention_dim=64, num_layers=2, dropout=0.3, bidirectional=True, target_length=750)
        assert fused_feature in ("mfb", "mfcc")
        self.fused_feature = fused_feature
    ...
    def forward(self, mfb_features: torch.Tensor, wav2vec2_features: torch.Tensor, mfcc_features: torch.Tensor):
        ...
        fused = torch.cat([wav2vec2_attended, low_level_attended], dim=-1)
        fused = self.dropout2(fused)
        logits = self.gru(fused, return_repr=False)
        return break_probs, logits
